...
import { CategoryComponent } from './components/task-manager/category/category.component';
...
describe('reducers', () => {
  ...
  test('should return the current category data', () => {
    expect(
      appReducer(initialState, AppActions.getCurrentCategoryData({ id: 1 }))
    ).toHaveProperty('currentCategoryData.id', 1);
  });

  test.only('should remove the category data', () => {
    ...
    expect(
      appReducer(initialState, AppActions.removeCategory({ id: 1 }))
    ).toHaveProperty('category', expectedCategory);
  });
  ...
  test('should return current task', () => {
    ...
    expect(
      appReducer(initialState, AppActions.getCurrentTaskById({ id: '101' }))
    ).toHaveProperty('currentTask', expectedCurrentTask);
  });
});
...
describe('actions', () => {
  test('create action [App] Get Current Category Data', () => {
    ...
    expect(AppActions.getCurrentCategoryData({ id: 1 })).toEqual(
      expectedAction
    );
  });
  ...
  test('create action [App] Get Current Task By Id', () => {
    ...
    expect(AppActions.getCurrentTaskById({ id: '101' })).toEqual(
      expectedAction
    );
  });
});
